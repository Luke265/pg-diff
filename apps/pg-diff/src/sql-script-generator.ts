...
export default {
   ...
   */
  __generateColumnDataTypeDefinition: function (columnSchema: any) {
   ...
   * @param {Object} columnSchema
   */
  __generateColumnDefinition: function (column: string, columnSchema: any) {
   ...
   */
  __generateTableGrantsDefinition: function (
    table: string,
    role: string,
    privileges: any
  ) {
   ...
   * @param {"f"|"p"} type
   */
  __generateProcedureGrantsDefinition: function (
    procedure: string,
    argTypes: string,
    role: string,
    privileges: any,
    ...
    if (privileges.execute)
      definitions.push(
        `GRANT EXECUTE ON ${procedureType} ${procedure}(${argTypes}) TO ${role};${hints.potentialRoleMissing}`
      );
   ...
   * @param {Object} privileges
   */
  __generateSequenceGrantsDefinition: function (
    sequence: string,
    role: string,
    privileges: any
  ) {
   ...
   */
  generateChangeCommentScript: function (
    objectType: string,
    objectName: string,
    comment: string,
   ...
   * @param {String} owner
   */
  generateCreateSchemaScript: function (schema: string, owner: string) {
   ...
   * @param {String} table
   */
  generateDropTableScript: function (table: string) {
   ...
   * @param {Object} schema
   */
  generateCreateTableScript: function (table: string, schema: any) {
    //Generate columns script
    let columns: string[] = [];
    for (let column in schema.columns) {
      columns.push(
        this.__generateColumnDefinition(column, schema.columns[column])
      );
    }
    ...
    for (let constraint in schema.constraints) {
      columns.push(
        `CONSTRAINT ${constraint} ${schema.constraints[constraint].definition} `
      );
    }
    ...
    for (let role in schema.privileges) {
      privileges = privileges.concat(
        this.__generateTableGrantsDefinition(
          table,
          role,
          schema.privileges[role]
        )
      );
    }

    let columnsComment: string[] = [];
    for (let column in schema.columns) {
      columnsComment.push(
        this.generateChangeCommentScript(
          objectType.COLUMN,
          `${table}.${column}`,
          schema.columns[column].comment
        )
      );
    }

    let constraintsComment: string[] = [];
    for (let constraint in schema.constraints) {
      constraintsComment.push(
        this.generateChangeCommentScript(
          objectType.CONSTRAINT,
          constraint,
          schema.constraints[constraint].comment,
          table
        )
      );
    }

    let indexesComment: string[] = [];
    for (let index in schema.indexes) {
      indexesComment.push(
        this.generateChangeCommentScript(
          objectType.INDEX,
          `"${schema.indexes[index].schema}"."${index}"`,
          schema.indexes[index].comment
        )
      );
    }
   ...
   * @param {Object} schema
   */
  generateAddTableColumnScript: function (
    table: string,
    column: string,
    schema: any
  ) {
    let script = `\nALTER TABLE IF EXISTS ${table} ADD COLUMN IF NOT EXISTS ${this.__generateColumnDefinition(
      column,
      schema
    )};`;
   ...
   */
  generateChangeTableColumnScript: function (
    table: string,
    column: string,
    changes: any
  ) {
    ...
    if (Object.prototype.hasOwnProperty.call(changes, 'default'))
      definitions.push(
        `ALTER COLUMN ${column} ${changes.default ? 'SET' : 'DROP'} DEFAULT ${
          changes.default || ''
        }`
      );

    if (
      Object.prototype.hasOwnProperty.call(changes, 'identity') &&
      Object.prototype.hasOwnProperty.call(changes, 'isNewIdentity')
    ) {
   ...
   * @param {String} column
   */
  generateDropTableColumnScript: function (
    table: string,
    column: string,
    withoutHint = false
  ) {
    let script = `\nALTER TABLE IF EXISTS ${table} DROP COLUMN IF EXISTS ${column} CASCADE;${
   ...
   * @param {Object} schema
   */
  generateAddTableConstraintScript: function (
    table: string,
    constraint: string,
    schema: any
  ) {
   ...
   * @param {String} constraint
   */
  generateDropTableConstraintScript: function (
    table: string,
    constraint: string
  ) {
   ...
   */
  generateChangeTableOptionsScript: function (table: string, options: any) {
    let script = `\nALTER TABLE IF EXISTS ${table} SET ${
      options.withOids ? 'WITH' : 'WITHOUT'
    } OIDS;\n`;
    return script;
  },
  generateChangeIndexScript: function (index: string, definition: string) {
   ...
   * @param {String} index
   */
  generateDropIndexScript: function (index: string) {
    let script = `\nDROP INDEX IF EXISTS ${index};\n`;
    return script;
  },
  /**
   ...
   * @param {Object} privileges
   */
  generateTableRoleGrantsScript: function (
    table: string,
    role: string,
    privileges: any
  ) {
   ...
   * @param {Object} changes
   */
  generateChangesTableRoleGrantsScript: function (
    table: string,
    role: string,
    changes: any
  ) {
   ...
   * @param {String} owner
   */
  generateChangeTableOwnerScript: function (table: string, owner: string) {
   ...
   * @param {Object} schema
   */
  generateCreateViewScript: function (view: string, schema: any) {
    //Generate privileges script
    let privileges: string[] = [];
   ...
   * @param {String} view
   */
  generateDropViewScript: function (view: string) {
   ...
   * @param {Object} schema
   */
  generateCreateMaterializedViewScript: function (view: string, schema: any) {
    //Generate indexes script
    let indexes: string[] = [];
    for (let index in schema.indexes) {
   ...
   * @param {String} view
   */
  generateDropMaterializedViewScript: function (view: string) {
   ...
   * @param {"f"|"p"} type
   */
  generateCreateProcedureScript: function (procedure: string, schema: any) {
    const procedureType = schema.type === 'f' ? 'FUNCTION' : 'PROCEDURE';

    //Generate privileges script
    let privileges: string[] = [];
    privileges.push(
      `ALTER ${procedureType} ${procedure}(${schema.argTypes}) OWNER TO ${schema.owner};`
    );
    for (let role in schema.privileges) {
      privileges = privileges.concat(
        this.__generateProcedureGrantsDefinition(
          procedure,
          schema.argTypes,
          role,
          schema.privileges[role],
   ...
   * @param {Object} schema
   */
  generateCreateAggregateScript: function (aggregate: string, schema: any) {
    //Generate privileges script
    let privileges: string[] = [];
    privileges.push(
      `ALTER AGGREGATE ${aggregate}(${schema.argTypes}) OWNER TO ${schema.owner};`
    );
    for (let role in schema.privileges) {
      privileges = privileges.concat(
        this.__generateProcedureGrantsDefinition(
   ...
   * @param {Object} schema
   */
  generateChangeProcedureScript: function (procedure: string, schema: any) {
    const procedureType = schema.type === 'f' ? 'FUNCTION' : 'PROCEDURE';

    let script = `\nDROP ${procedureType} IF EXISTS ${procedure}(${
      schema.argTypes
   ...
   * @param {Object} schema
   */
  generateChangeAggregateScript: function (aggregate: string, schema: any) {
   ...
   */
  generateDropProcedureScript: function (
    procedure: string,
   ...
   */
  generateDropAggregateScript: function (
    aggregate: string,
   ...
   * @param {"f"|"p"} type
   */
  generateProcedureRoleGrantsScript: function (
    procedure: string,
    argTypes: string,
    role: string,
    privileges: any,
   ...
   * @param {"f"|"p"} type
   */
  generateChangesProcedureRoleGrantsScript: function (
    procedure: string,
    argTypes: string,
    role: string,
    changes: any,
    ...
    if (Object.prototype.hasOwnProperty.call(changes, 'execute'))
      privileges.push(
        `${
          changes.execute ? 'GRANT' : 'REVOKE'
        } EXECUTE ON ${procedureType} ${procedure}(${argTypes}) ${
   ...
   * @param {"p"!"f"} type
   */
  generateChangeProcedureOwnerScript: function (
    procedure: string,
    argTypes: string,
    owner: string,
    type: 'p' | 'f'
   ...
   * @param {String} owner
   */
  generateChangeAggregateOwnerScript: function (
    aggregate: string,
    argTypes: string,
    owner: string
  ) {
   ...
   * @param {Object} changes
   */
  generateUpdateTableRecordScript: function (
    table: string,
    fields: any,
    ...
    for (let field in changes) {
      updates.push(
        `"${field}" = ${this.__generateSqlFormattedValue(
          field,
          fields,
          changes[field]
        )}`
    ...
    for (let condition in filterConditions) {
      conditions.push(
        `"${condition}" = ${this.__generateSqlFormattedValue(
          condition,
          fields,
   ...
   */
  generateInsertTableRecordScript: function (
    table: string,
    record: any,
    fields: any,
    ...
    for (let field in record) {
      ...
      fieldValues.push(
        this.__generateSqlFormattedValue(field, fields, record[field])
      );
    }
   ...
   */
  generateDeleteTableRecordScript: function (
    table: string,
    fields: any,
    keyFieldsMap: any
  ) {
  ...
  __generateSqlFormattedValue: function (
    fieldName: string,
    fields: any,
    value: any
  ) {
    if (value === undefined)
      throw new Error(
        `The field "${fieldName}" contains an "undefined" value!`
      );
  ...
  generateMergeTableRecord(
    ...
    for (let field in changes) {
      fieldNames.push(`"${field}"`);
      fieldValues.push(
        this.__generateSqlFormattedValue(field, fields, changes[field])
      );
      updates.push(
        `"${field}" = ${this.__generateSqlFormattedValue(
          field,
  ...
  generateSetSequenceValueScript(tableName: string, sequence: any) {
  ...
  generateChangesSequenceRoleGrantsScript: function (
    sequence: string,
    role: string,
    changes: any
  ) {
    ...
    if (Object.prototype.hasOwnProperty.call(changes, 'select'))
      privileges.push(
        `${
          changes.select ? 'GRANT' : 'REVOKE'
        } SELECT ON SEQUENCE ${sequence} ${
          changes.select ? 'TO' : 'FROM'
        } ${role};${hints.potentialRoleMissing}`
      );
    ...
    if (Object.prototype.hasOwnProperty.call(changes, 'update'))
      privileges.push(
        `${
          changes.update ? 'GRANT' : 'REVOKE'
        } UPDATE ON SEQUENCE ${sequence} ${
          changes.update ? 'TO' : 'FROM'
        } ${role};${hints.potentialRoleMissing}`
      );
  ...
  generateSequenceRoleGrantsScript: function (
    sequence: string,
    role: string,
    privileges: any
  ) {
    let script = `\n${this.__generateSequenceGrantsDefinition(
      sequence,
      role,
      privileges
    ).join('\n')}`;
  ...
  generateCreateSequenceScript: function (sequence: string, schema: any) {
    //Generate privileges script
    let privileges: string[] = [];
...
\n${privileges.join('\n')}\n`;
  ...
  generateRenameSequenceScript: function (old_name: string, new_name: string) {
