...
const query = {
  ...
  getSchemas: function (schemas: string[]) {
    //TODO: Instead of using ::regrole casting, for better performance join with pg_roles
  ...
  getTables: function (schemas: string[]) {
  ...
  getTableOptions: function (schemaName: string, tableName: string) {
  ...
  getTableConstraints: function (schemaName: string, tableName: string) {
  ...
  getTableIndexes: function (schemaName: string, tableName: string) {
  ...
  getTablePrivileges: function (schemaName: string, tableName: string) {
  ...
  getViews: function (schemas: string[]) {
  ...
  getViewPrivileges: function (schemaName: string, viewName: string) {
  ...
  getMaterializedViews: function (schemas: string[]) {
  ...
  getMaterializedViewPrivileges: function (
    schemaName: string,
    viewName: string
  ) {
   ...
   */
  getViewDependencies: function (schemaName: string, viewName: string) {
    return `SELECT                 
  ...
  getFunctions: function (schemas: string[], serverVersion: ServerVersion) {
    //TODO: Instead of using ::regrole casting, for better performance join with pg_roles
  ...
  getAggregates: function (schemas: string[], serverVersion: ServerVersion) {
    //TODO: Instead of using ::regrole casting, for better performance join with pg_roles
  ...
  getFunctionPrivileges: function (
    schemaName: string,
    functionName: string,
    argTypes: string
  ) {
  ...
  getSequences: function (schemas: string[], serverVersion: ServerVersion) {
  ...
  getSequencePrivileges: function (
    schemaName: string,
    sequenceName: string,
    serverVersion: ServerVersion
  ) {
...
export class CatalogApi {
  ...
  static async retrieveSchemas(client: ClientBase, schemas: string[]) {
  ...
  static async retrieveTables(client: ClientBase, config: Config) {
    let result: Record<string, Table> = {};
    let tableNamesPriority: string[] = [];

    const tables = await client.query(
      query.getTables(config.compareOptions.schemaCompare.namespaces)
    );
    ...
    await Promise.all(
      tables.rows.map(async (table) => {
        ...
        let constraints = await client.query(
          query.getTableConstraints(table.schemaname, table.tablename)
        );
        ...
        if (
          !core.checkServerCompatibility(await getServerVersion(client), 12, 0)
        ) {
          let options = await client.query(
            query.getTableOptions(table.schemaname, table.tablename)
          );
        ...
        let indexes = await client.query(
          query.getTableIndexes(table.schemaname, table.tablename)
        );
        ...
        let privileges = await client.query(
          query.getTablePrivileges(table.schemaname, table.tablename)
        );
    ...
    const reorderedResult: Record<string, any> = {};
  ...
  static async retrieveViews(client: ClientBase, config: Config) {
    ...
    const views = await client.query(
      query.getViews(config.compareOptions.schemaCompare.namespaces)
    );
    ...
    await Promise.all(
      views.rows.map(async (view) => {
        ...
        let privileges = await client.query(
          query.getViewPrivileges(view.schemaname, view.viewname)
        );
        privileges.rows.forEach((privilege) => {
          if (
            config.compareOptions.schemaCompare.roles.length <= 0 ||
        ...
        let dependencies = await client.query(
          query.getViewDependencies(view.schemaname, view.viewname)
        );
  ...
  static async retrieveMaterializedViews(client: ClientBase, config: Config) {
    ...
    const views = await client.query(
      query.getMaterializedViews(config.compareOptions.schemaCompare.namespaces)
    );
    ...
    await Promise.all(
      views.rows.map(async (view) => {
        ...
        let indexes = await client.query(
          query.getTableIndexes(view.schemaname, view.matviewname)
        );
        indexes.rows.forEach((index) => {
        ...
        let privileges = await client.query(
          query.getMaterializedViewPrivileges(view.schemaname, view.matviewname)
        );
        privileges.rows.forEach((privilege) => {
          if (
            config.compareOptions.schemaCompare.roles.length <= 0 ||
        ...
        let dependencies = await client.query(
          query.getViewDependencies(view.schemaname, view.matviewname)
        );
        dependencies.rows.forEach((dependency) => {
          result[fullViewName].dependencies.push({
            schemaName: dependency.schemaname,
  ...
  static async retrieveAggregates(client: ClientBase, config: Config) {
    ...
    await Promise.all(
      aggregates.rows.map(async (aggregate) => {
        ...
        const aggregateObj = (result[fullAggregateName] ??= {});

        aggregateObj[aggregate.argtypes] = {
        ...
        privileges.rows.forEach((privilege: any) => {
          if (
            config.compareOptions.schemaCompare.roles.length <= 0 ||
            config.compareOptions.schemaCompare.roles.includes(
              privilege.usename
            ...
            aggregateObj[aggregate.argtypes].privileges[privilege.usename] = {
              execute: privilege.execute,
            };
        });
      })
